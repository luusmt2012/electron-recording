...
const videoClient = document.querySelector('#streamClient');
...
async function startCapture() {
	try {
		...
		videoClient.srcObject = await navigator.mediaDevices.getDisplayMedia(
			displayMediaOptions
		);
...
function stopCapture(evt) {
	...
	if (!videoClient.srcObject) return;
	
	let tracks = video.srcObject.getTracks();
	let tracks1 = videoClient.srcObject.getTracks();
